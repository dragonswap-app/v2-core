...
contract TestDragonswapV2Router is IDragonswapV2SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IDragonswapV2Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function dragonswapV2SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IDragonswapV2Pool(msg.sender).token0() : IDragonswapV2Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IDragonswapV2Pool(pools[0]).token1();
            IDragonswapV2Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
